...
module.exports = function(grunt) {
        ...
        copy: {
            ...
            release: {
                files: [{
                    ...
                    dest: path.resolve('<%= paths.dist %>/node-kishore-<%= pkg.version %>')
                }]
            }
        },
        chmod: {
            ...
            release: {
                ...
                src: [
                    path.resolve('<%= paths.dist %>/node-kishore-<%= pkg.version %>/nodes/core/hardware/nrgpio*')
                ]
            }
        },
        compress: {
            release: {
                options: {
                    archive: '<%= paths.dist %>/node-kishore-<%= pkg.version %>.zip'
                },
                expand: true,
                cwd: '<%= paths.dist %>/',
                src: ['node-kishore-<%= pkg.version %>/**']
            }
        }
    });
    ...
    grunt.registerTask('dev',
        'Developer mode: run node-kishore, watch for source changes and build/restart',
